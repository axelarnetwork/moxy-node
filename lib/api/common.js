...
const handleNumberResponse = (getFromNode, method, args, callback, overrides = {}, options) =>
  getFromNode(method, args, (err, response) =>
    response.error
...
const handleObjectResponse = (getFromNode, method, args, callback, overrides = {}, options) =>
  getFromNode(method, args, (err, response) =>
    response.error
      ? callback(response.error, null)
      : callback(
          ...
          Object.entries(overrides).reduce(
            (newResult, [property, override]) =>
              Object.assign(newResult, {
                [property]: overrideProperty(response.result?.[property], override, options),
              }),
            response.result
          )
        )
...
const getNumber = (getFromNode, transientState, method, property, options) => (time) => (args, callback) =>
  handleNumberResponse(getFromNode, method, args, callback, getLastState(transientState, time, property), options);

// Performs an rpc call, overriding if necessary, where just a number is expected as a response result
const getNumberFromArg = (getFromNode, transientState, method, index = 0, options) => (time) => (args, callback) =>
  handleNumberResponse(getFromNode, method, args, callback, getLastState(transientState, time, args[index]), options);
