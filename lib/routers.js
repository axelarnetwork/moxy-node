...
const proxyOverrideCallback = (method, params, transientState, time, err, response, callback) => {
  ...
  callback(proxyResponse.error ?? null, proxyResponse.response);
};
...
const getRequestRouter = (startTime, transientState, getFromNode) => (method, params) => {
  ...
  console.log(`Routing ${method} call from client.`);
  ...
  return (params, callback) =>
    getFromNode(method, params, (err, response) => {
      if (err) {
        console.log(`Error overriding not yet supported (${time}).`);
        callback(err);
      ...
      if (!nodeMethods[method]) {
        ...
        response.error ? callback(response.error) : callback(null, response.result);
